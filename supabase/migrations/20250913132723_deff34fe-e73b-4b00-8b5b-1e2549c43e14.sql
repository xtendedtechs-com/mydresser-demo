...
-- Fixed version that properly handles function signature changes
...
-- Step 2: Drop and recreate the get_public_profiles function with the correct signature
...
-- Step 3: Create a secure view that only exposes non-sensitive public profile fields
...
USING (
  ...
  (auth.uid() = user_id) 
  OR 
  -- For other users' public profiles, limit access - applications should use secure functions/views
  (is_profile_public = true AND auth.uid() IS NOT NULL AND auth.uid() != user_id)
);

-- Step 5: Create a function to get safe public profile data for a specific user
CREATE FUNCTION public.get_public_profile_by_user_id(target_user_id uuid)
RETURNS TABLE(
  id uuid, 
  user_id uuid, 
  full_name text, 
...
-- Step 6: Grant permissions for the new functions and view
...
IS 'SECURITY: Returns all public profiles with only safe, non-sensitive fields (excludes email, location, social_instagram, social_facebook, social_tiktok, privacy_settings). Use this instead of direct table queries for public profile access.';

COMMENT ON FUNCTION public.get_public_profile_by_user_id(uuid) 
IS 'SECURITY: Returns a specific public profile with only safe fields. Excludes sensitive data like email addresses and social media handles.';

COMMENT ON VIEW public.profiles_public_safe 
IS 'SECURITY: Safe view of public profiles that excludes sensitive fields like email, location, and social media handles. Use this for UI components that display public profiles.';

COMMENT ON POLICY "Safe public profile viewing" ON public.profiles 
IS 'SECURITY: Allows authenticated users to view public profiles. Applications MUST use get_public_profiles() function or profiles_public_safe view to ensure sensitive fields like email addresses are not exposed.';